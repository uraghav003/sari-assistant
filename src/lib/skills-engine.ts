...
export const seedSkills: Skill[] = [
  ...
  },
  {
    ...
    status: "live",
    trigger: "laila / audit / zero-trust / compliance",
    steps: ["Scan headers", "Validate auth token", "Check webhook telemetry", "Log compliance heartbeat"],
    instruction: "Run strict Zero-Trust policy compliance check and flag unauthorized access vectors.",
    runs: 0,
  },
  {
    id: "bulbul-dispatch",
    name: "BULBUL Autonomous Dispatch",
    description: "Sub-agent BULBUL autonomous lead dispatch and webhook routing for Divyanshi Capital",
    status: "live",
    trigger: "bulbul / dispatch / webhook / lead",
    steps: ["Ingest lead payload", "Sanitize PII", "Route to lender CRM", "Confirm webhook ACK"],
    instruction: "Dispatch incoming lead packets to verified lender endpoints with zero PII leakage.",
    runs: 0,
  },
  {
    id: "lead-brief",
